...
{
//    выгрузка товара в таблицу order
...
//    сохранение продукта заказа
    public static function saveOrderProduct($order_id) {
        $sql_part = '';
        foreach ($_SESSION['cart'] as $product_id => $product) {
            $product_id = (int)$product_id; // убирает второй номер модификации
            $sql_part .= "($order_id, $product_id, {$product['qty']}, '{$product['title']}', {$product['price']}),";
        }
        $sql_part = rtrim($sql_part, ','); // удаляет последнюю запятую
        \R::exec("INSERT INTO order_product (order_id, product_id, qty, title, price) VALUES $sql_part");
    }

//    отправляет письмо администратору (swiftmailer)
    public static function mailOrder($order_id, $user_email) {
        ...
        $message_admin = (new Swift_Message("Сделан заказ № {$order_id} на сайте"))
            ->setFrom([App::$app->getProperty('smtp_login') => App::$app->getProperty('shop_name')])
